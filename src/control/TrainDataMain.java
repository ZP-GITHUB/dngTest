 ...
 */
public class TrainDataMain {

    private static SqlSession openSession = MyBatisUtil.getSession();

    private static RoadDao roadDao = openSession.getMapper(RoadDao.class);


    private static OriginDataDao originDataDao = openSession.getMapper(OriginDataDao.class);


    private static TrainDataDao trainDataDao = openSession.getMapper(TrainDataDao.class);

    /**
     * TrainData缓存
     */
    static Map<String, TrainData> trainDataMap = new LinkedHashMap<String, TrainData>();

    static List<Road> roads;

    static Map<Long, List<OriginData>> listMap = new HashMap<Long, List<OriginData>>();
    ...
    public static void main(String[] args) throws ParseException {
         ...
         * 结束时间
         */
        long endTime = sf.parse("2016-10-31 23:59:59").getTime() / 1000L;
        ...
        while (iterator.hasNext()) {
            ...
            new Thread(new TrainDataMain.countThread(data.getRoadId(), data.getStartTime(), data.getEndTime(), countDownLatch)).start();
            count++;
            if (count == 8) {
                try {
            ...
            for (int x = 0; x < originData.size(); x++) {
                ...
                if (this.roadId.equals(roadId)) {
                    if (set.contains(o.getDeviceId())) {
                        ...
                        synchronized (TrainDataMain.countThread.class) {
